  ...
  return (
    <span
      className={cn(
        ...
        // Exactly one font-size class per stage: twMerge doesn't know text-sm,
        // so a base size + override would both survive the merge.
        stage === 'system' ? 'font-mono text-sm tracking-[0.1em]' : 'text-sm tracking-wide',
        config.textClass,
      )}
    >
...
function GroupIdentity({ group, runEvent }: { group: LogSiteGroup; runEvent: boolean }) {
  return (
    <div className="flex min-w-0 items-center gap-2">
      ...
      {runEvent ? (
        <span className="block truncate text-sm font-medium text-secondary" title={group.label}>
          {group.label}
        </span>
      ) : (
        <a
          ...
          className="block truncate text-sm font-normal text-info-text underline-offset-4 hover:underline"
          title={group.url}
        >
...
function GroupMetrics({
  ...
  return (
    <>
      <div
        className="flex items-center gap-1 rounded-md border border-border bg-background px-1.5 py-0.5 text-sm font-medium whitespace-nowrap text-secondary tabular-nums"
...
function ExpandedGroupRows({
  ...
  if (!rows.length)
    return <div className="px-3 py-2 text-sm opacity-40">{TERMINAL_STRINGS.NO_LOGS}</div>;
  return rows.map((row, index) => {
    ...
    return (
      <div
        key={row.key}
        className={cn(
          'grid grid-cols-[64px_24px_105px_minmax(0,1fr)_auto] items-center gap-4 px-6 py-0.5 text-sm',
          index % 2 === 0
      ...
      >
        ...
        <span className="min-w-0 text-sm font-medium break-words text-secondary">
            ...
            {visibleGroups.map((group, index) => {
              ...
              return (
                <section key={group.key} id={siteDomId(group.key)} className="overflow-hidden">
                  <div
                    className={cn(
                      'group/row font-inherit grid w-full items-center gap-3 border-none bg-transparent px-6 py-2 text-left text-sm text-inherit transition-colors',
                      isRunEventGroup
                    ...
                    {!isRunEventGroup ? (
                      <div className="flex items-center justify-end">
                        {payload ? (
                          ...
                          <span className="type-caption text-sm opacity-25">--</span>
                        )}
                      </div>
                    ) : null}
                    <GroupToggle
          ...
          >
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                <div
                  className="mt-0.5 truncate pr-4 text-sm font-medium tabular-nums"
