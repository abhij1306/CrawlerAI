...
export const appRoutes = [
  ...
  {
    id: 'register', path: '/register', access: 'public',
    lazy: () => import('../../app/register/page-view'), metadata: { title: 'Register' },
  },
  {
    id: 'dashboard', path: '/dashboard', access: 'authenticated',
    lazy: () => import('../../app/dashboard/page-view'),
    metadata: { title: 'Dashboard', description: 'Overview of crawler activity across your workspace.' },
    nav: { group: 'Primary', label: 'Dashboard', icon: Grid2x2 },
  },
  {
    id: 'crawl', path: '/crawl', access: 'authenticated',
    lazy: () => import('../../app/crawl/page-view'),
    metadata: { title: 'Crawl Studio', description: 'Configure sources, run jobs, and monitor execution.' },
    nav: { group: 'Primary', label: 'Crawl Studio', icon: WandSparkles },
  },
  {
    id: 'runs', path: '/runs', access: 'authenticated',
    lazy: () => import('../../app/runs/page-view'),
    metadata: { title: 'Run History', description: 'Review and manage previously submitted crawls.' },
    nav: { group: 'Primary', label: 'History', icon: Clock3 },
  },
  {
    id: 'jobs', path: '/jobs', access: 'authenticated',
  ...
  {
    id: 'run-trace', path: '/run-trace', access: 'authenticated',
    lazy: () => import('../../app/run-trace/page-view'),
    metadata: { title: 'Run Trace', description: 'Inspect acquire timeline, extraction tiers, and auto-flagged bugs for a run.' },
    nav: { group: 'Operations', label: 'Run Trace', icon: Network },
  },
  {
    id: 'data-enrichment', path: '/data-enrichment', access: 'authenticated',
    lazy: () => import('../../app/data-enrichment/page-view'),
    metadata: { title: 'Data Enrichment', description: 'Normalize ecommerce detail records into discovery fields.' },
    nav: { group: 'Intelligence', label: 'Data Enrichment', icon: FileChartColumn },
  },
  {
    id: 'product-intelligence', path: '/product-intelligence', access: 'authenticated',
    lazy: () => import('../../app/product-intelligence/page-view'),
    metadata: { title: 'Product Intelligence', description: 'Find matching product pages and compare prices.' },
    nav: { group: 'Intelligence', label: 'Product Intelligence', icon: BrainCircuit },
  },
  {
    id: 'selectors', path: '/selectors', access: 'authenticated',
    lazy: () => import('../../app/selectors/page-view'),
    metadata: { title: 'Selector Tool', description: 'Suggest, test, and validate field selectors.' },
    nav: { group: 'Memory', label: 'Selector Tool', icon: SearchCheck, exact: true },
  },
  {
    id: 'domain-memory', path: '/domain-memory', access: 'authenticated',
    lazy: () => import('../../app/domain-memory/page-view'),
    metadata: { title: 'Domain Memory', description: 'Inspect learned selectors and saved run profiles by domain and surface.' },
    nav: { group: 'Memory', label: 'Domain Memory', icon: DatabaseZap },
  },
  {
    id: 'admin-users', path: '/admin/users', access: 'admin',
  ...
  {
    id: 'admin-llm', path: '/admin/llm', access: 'admin',
