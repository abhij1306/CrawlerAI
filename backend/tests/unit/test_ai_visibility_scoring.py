"""Slice 1: deterministic mention/citation/domain/fanout scoring."""

from __future__ import annotations

import pytest

from app.ai_visibility.constants import BEST_AND_LESS_PROJECT
from app.ai_visibility.scoring import (
    ScoringConfig,
    aggregate_run,
    classify_citation,
    classify_fanout,
...
def test_aggregate_run_rates_and_stability() -> None:
    config = _config()
    # prompt 0: 3 reps, brand mentioned in all 3, owned cited in 2
    # prompt 1: 3 reps, brand mentioned in 1, owned cited in 0
    executions = []
    for rep in range(3):
        executions.append(
            {
    ...
    assert prompt0["mention_stability"] == 1.0  # 3/3
    assert prompt0["owned_cited_count"] == 2  # 2/3

    prompt1 = next(p for p in summary["per_prompt"] if p["prompt_index"] == 1)
    assert prompt1["brand_mentioned_count"] == 1
    assert prompt1["mention_stability"] == round(2 / 3, 4)  # 2 of 3 are "false"
...
def test_aggregate_run_token_usage_defaults_to_zero() -> None:
    """Executions with no provider usage block report a stable zeroed shape."""
    config = _config()
