...
DETAIL_ARTIFACT_IDENTIFIER_VALUES = frozenset({"description", "details", "product details", "specification", "specifications"})
DETAIL_ARTIFACT_PRICE_VALUES = frozenset({"free", "n/a", "na", "unavailable", "contact us"})
...
DETAIL_LONG_TEXT_LEADING_ATTRIBUTE_BLOB_PATTERN = r"^(?:[a-zA-Z][\w:-]*\s*=\s*(?:\"[^\"]*\"|'[^']*')\s*){1,8}"
...
DETAIL_MATERIALS_COMPOSITION_PATTERN = r"\d{1,3}\s*%\s*[A-Za-z][A-Za-z\u00C0-\u017F\s\-]{2,40}"
...
DETAIL_BRAND_SUFFIX_REJECT_TOKENS = frozenset({"lifewear", "vintage watches", "official store"})
...
DETAIL_VARIANT_ARTIFACT_VALUE_TOKENS = frozenset({"discount", "false", "off", "on", "sale", "true"})
...
NOISY_PRODUCT_ATTRIBUTE_KEYS = frozenset(tuple(_STATIC_EXPORTS.get("NOISY_PRODUCT_ATTRIBUTE_KEYS", ()) or ())) | frozenset(
    {
        "availability",
        "available",
