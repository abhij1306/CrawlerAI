...
def publish_recipe_execution(
    ...
    """Project authorized recipe-executor values. No discovery or repair."""

    model: Any = _MODEL_BY_SURFACE[request.surface]
    field_sources = _recipe_field_sources(recipe, execution)
    records: list[PublicRecord] = []
    for index, raw in enumerate(execution.records[: request.max_records]):
        lineage = _recipe_lineage(recipe, execution, record_index=index)
        payload = _recipe_public_payload(model, raw)
        if model is CommerceDetailRecord:
            ...
            variants = tuple(
                CommerceVariantRecord.model_validate(
                    {key: value for key, value in row.items() if key != "_binding_lineage"}
                )
    ...
    findings = (
        validate_selected_contract_fields(result, request.requested_fields)
...
def _recipe_public_payload(model: Any, raw: dict[str, object]) -> dict[str, object]:
