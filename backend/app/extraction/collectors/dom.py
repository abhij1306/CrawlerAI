...
def _direct_variant_rows(
    ...
    *,
    ...
    variant_key = _first_node_attribute(
        node, VARIANT_DOM_DIRECT_ID_ATTRIBUTES
    ) or "|".join(f"{axis}:{value}" for axis, value in sorted(options.items()))
    variant_subject = stable_id(
        "subject", bundle.bundle_id, "dom", "variant", variant_key
    )
    ...
    rows = [
        ...
        for fact_type, value in (
            *(
                (f"variant.option.{axis}", value)
                for axis, value in sorted(options.items())
            ),
...
def _direct_variant_offer_rows(
    ...
    *,
    ...
    offer_fields = (
        (
            "offer.price",
            _first_node_attribute(node, VARIANT_DOM_DIRECT_PRICE_ATTRIBUTES),
        ),
        (
            "offer.currency",
    ...
    return [
        evidence(
            ...
            subject_id=stable_id(
                "subject", bundle.bundle_id, "dom", "offer", variant_key
            ),
            parent_subject_id=variant_subject,
            parent_scope="variant",
        )
