...
def build_entities(bundle: CaptureBundle, evidence: tuple[Evidence, ...]) -> EntitySet:
    ...
    return EntitySet(products=products, variants=variants, offers=offers, assets=assets, option_catalogs=option_catalogs)
...
def _link_variants(evidence: tuple[Evidence, ...], product_by_subject: dict[str, str]) -> tuple[VariantEntity, ...]:
    return tuple(
        ...
        for product_id, keys, rows, source_subjects in _variant_groups(evidence, product_by_subject)
    )
...
def _variant_groups(
    ...
    return sorted(zip(product_ids, group_keys, groups, subjects), key=lambda item: (item[0], sorted(item[1])))
...
def _variant_for(rows: list[Evidence], variants: tuple[VariantEntity, ...]) -> str | None:
    ...
    for variant in variants:
        if any(str(item).split(":", 1)[-1] in skus for item in (variant.identity_key,)):
            return variant.entity_id
    return None
def _product_for_child(rows: list[Evidence], product_by_subject: dict[str, str], variants: tuple[VariantEntity, ...], variant_id: str | None) -> str | None:
    if variant_id:
        return next(
            (
