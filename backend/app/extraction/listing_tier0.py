...
def collect_deterministic_listing(
    bundle: CaptureBundle,
    reader: ArtifactReader,
    *,
    surface: Surface,
) -> list[Evidence]:
    """Return the selector-free deterministic floor for a many-record surface.

    Structured evidence is preferred.  When it is absent, the floor keeps only
    repeated record boundaries with a record-local title and the boundary URL.
    It never promotes one-off navigation or footer links into a listing.
    """
    structured = collect_structured_listing(bundle, reader, surface=surface)
    if structured:
        return structured
    network = collect_network_listing(bundle, reader, surface=surface)
    if network:
        return network
