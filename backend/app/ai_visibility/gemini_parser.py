...
Field shapes were confirmed against a live probe (see the saved fixture
``backend/tests/fixtures/gemini_interactions_grounded.json``), not from docs.
...
Key facts used here:
  * The citation ``url`` is a Google grounding-redirect URL, NOT the publisher
    URL. The publisher domain is carried in ``title``. We therefore derive the
    citation domain from ``title``.
...
def sanitize_metadata(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    Retains status/model/usage/object and a redacted evidence envelope containing
    search-call grouping, model output, and citation annotations. Strips thought
    steps/signatures and never carries credentials.
    """
    steps = [
        step
    ...
    for step in steps:
        ...
        if step_type == "google_search_call":
            ...
            evidence_steps.append(
                {**common, "arguments": {"queries": arguments.get("queries") or []}}
            )
