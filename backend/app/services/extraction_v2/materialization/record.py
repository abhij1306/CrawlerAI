...
from app.services.extraction_v2.contracts import Evidence, ResolutionResult
from app.services.extraction_v2.entities.contracts import EntitySet
from app.services.extraction_v2.materialization.lineage import lineage
...
def materialize(entities: EntitySet, resolution: ResolutionResult, evidence: tuple[Evidence, ...]) -> dict[str, object]:
    ...
    variants = _variants(entities, resolution, by_id)
    if variants:
...
def _variants(entities: EntitySet, resolution: ResolutionResult, by_id: dict[str, Evidence]) -> list[dict[str, object]]:
