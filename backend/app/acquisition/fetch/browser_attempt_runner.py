...
class BrowserAttemptRunner:
    ...
    async def run(self) -> PageFetchResult:
        ...
        for proxy_index, proxy in enumerate(
            ...
            if result is not None:
                return result
            # A failed browser attempt does not rotate into another browser.
            # Only a blocked headless result may use real Chrome below.
    ...
    async def _run_proxy_attempt(
        ...
        while engine_index < len(engine_attempts):
            ...
            if (
                engine != "patchright"
                or not self.plan.attempt_results
                or self.plan.attempt_results[-1].outcome != "blocked"
            ):
                break
            engine_attempts = (
            ...
            if (
                engine_index < len(engine_attempts)
                and engine_attempts[engine_index] != "real_chrome"
            ):
                break
        return None
