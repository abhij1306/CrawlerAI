...
- Slice 8 added blocking backend test/tool CC 15 and exact nonblank LOC ratchets (tests 55,357; root/browser-probe tools 3,723), frontend test complexity 15 and nonblank LOC ratchet (4,642), full-backend Ruff CI scope, and frontend lint/type/policy CI before build. Backend Ruff, mypy (362 files), 63 architecture tests, and Alembic check pass. Frontend `vp check`, all architecture policies, and production build pass. CI database service names now use canonical `crawlerai` in both workflows. The ratchet later increased only for explicit shared-test export contracts and failure-safe cleanup added during CodeQL closeout.
