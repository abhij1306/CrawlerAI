...
- public internal-API replay endpoints are typed bounded exact-route memory, verified anonymously before persistence, and evicted after consecutive replay failures. Replay remains disabled by default until controlled and live parity is proven; stored candidates retain only safe admission metadata. Replayed JSON enters extraction as `network_json` with empty HTML.
...
- commerce and job listings now share a schema-backed cascade: repeated structural record boundaries, deterministic structured or record-local DOM evidence, and accepted network rows. Shared model fallback is eligible only after a real boundary exists; it cannot invent record identity or URLs
