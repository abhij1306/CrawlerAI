...
- public internal-API replay endpoints are stored as bounded exact-route memory, verified anonymously before persistence, and evicted after consecutive replay failures. They preserve POST request templates and route-significant query parameters; replayed JSON enters extraction as `network_json` with empty HTML.
